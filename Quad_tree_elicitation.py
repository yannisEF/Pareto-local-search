            ...
            for _ in range(self.nb_tries):
                ...
                #for i in range(1):
                    ...
                    if len(pareto_front) != 1 :
                        ...
                        user = elicitation.DecisionMaker(weighted_sum, len(pareto_front[0]), self.hiden_weight)
                        elicitor = elicitation.Elicitor(pareto_front, user)
                        (elicitor.query_user())
                        N_question += elicitor.number_of_question
...
if __name__ == "__main__":
    ...
    save_name = root.split('/')[-1][:-4].format(0) + "_obj={}_crit={}"
    pls1 = PLS_QT((0.5,0.0,0.0,0.0,0.0,0.5),nb_tries=1, nb_files=1)  
    pls1.run()        
